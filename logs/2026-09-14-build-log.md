...
## Next: Step 3c — POS web app (`apps/pos`)

**Plan**
- Next.js app with:
  - device sign-in; lock screen with staff tiles and PIN pad; idle lock
  - floor grid with live timers and running prices (engine preview), polling
  - open walk-in; close dialog: member scan/search, referral, free minutes, quote, override with approver PIN, cash (change) / card, receipt print
  - today's bookings: arrive / no-show
  - shift open / paid out / close with report
- USB scanner input (keyboard wedge) on the close dialog.

**Verify**
- Build + typecheck + lint (first ESLint setup).
- Drive the real app in a browser against the local API and DB: sign in, PIN, open shift, walk-in, close with member, print receipt, close shift. Screenshot evidence.
